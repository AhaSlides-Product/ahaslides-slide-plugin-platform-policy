...
- **Your QA reviewer is fixed per slide type.** Each slide type has one permanent QA reviewer -
 either Lily or Amber. One of the two is picked **at random** the first time the slide type is
 submitted, and then never changes: every later submission and resubmission of that same slide type
 goes to the **same** QA reviewer, so they build up context on your slide type across versions.
 UI / UX (Lan) and Marketing (Trent) are single reviewers and are unaffected.
...
Each slide type has a permanent QA reviewer (Lily or Amber), picked at random at first submission and
unchanged for every later submission of that slide type (section 4).
...
| 1.4 | 22 September 2026 | QA reviewer is now fixed per slide type: each slide type has one permanent QA reviewer (Lily or Amber), picked at random at first submission and unchanged for every later submission of that slide type. Enforced in the review portal. UI/UX and Marketing reviewers unaffected. | Pending - Dave |
